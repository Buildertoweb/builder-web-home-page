      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="bg-white p-8 rounded-xl border border-gray-100 shadow-md">
              <div className="flex mb-4">
                {[...Array(testimonial.stars)].map((_, i) => (
                  <Star key={i} className="h-5 w-5 text-yellow-400 fill-current" />
                ))}
              </div>
              <blockquote className="text-lg text-gray-700 mb-6">"{testimonial.quote}"</blockquote>
              <div className="flex items-center">
                <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-purple-400 rounded-full flex items-center justify-center text-white font-medium text-lg">
        ...
        <div className="mt-16 bg-blue-50 rounded-2xl p-8 md:p-12">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="mb-8 md:mb-0 md:mr-8 text-center md:text-left">
              <div className="text-blue-600 font-semibold mb-2">TRUSTED BY BUSINESSES</div>
              <h3 className="text-2xl md:text-3xl font-bold">Join over 10,000+ companies using BuilderToWeb</h3>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {['Brand 1', 'Brand 2', 'Brand 3', 'Brand 4'].map((brand, i) => (
                <div key={i} className="h-12 bg-white/50 rounded flex items-center justify-center px-6">
